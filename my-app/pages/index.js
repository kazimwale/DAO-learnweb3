...
export default function Home() {
    ...
    const [treasuryBalance, setreasuryBalance] = useState("0")
    ...
    // Helper function to return a CryptoDevs DAO Contract instance
    const getDaoContractInstance = providerOrSigner => {
        return new Contract(CRYPTODEVS_DAO_ABI, CRYPTODEVS_DAO_CONTRACT_ADDRESS, providerOrSigner)
    }
    ...
    // helper function that try connect the wallet
    ...
    const getDAOTreasuryBalance = async () => {
        try {
            ...
            setreasuryBalance(balance.toString())
        } catch (err) {
            console.log(err)
        }
    }
    ...
    // read the balance of the  user that hold the nfts
    ...
    const createProposal = async () => {
        try {
            const signer = await getProviderOrSigner(true)
            const daoContract = new getDaoContractInstance(signer)
    ...
    const fetchAllProposalId = async () => {
        try {
            const proposals = []
    ...
    const voteOnProposal = async (_vote, proposalId) => {
        try {
            const signer = await getProviderOrSigner(true)
            const DaoContract = new getDaoContractInstance(signer)
